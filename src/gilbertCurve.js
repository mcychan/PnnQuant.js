	...
	function ditherPixel(x, y)
	{
		...
		if(saliencies != null && !sortedByYDiff) {
			...
			if (nMaxColors < 3 || margin > 6) {
				if (nMaxColors > 8 && (Y_Diff(r0, g0, b0, r1, g1, b1) > (beta * acceptedDiff) || U_Diff(r0, g0, b0, r1, g1, b1) > (2 * acceptedDiff))) {
				...
				if(beta < .3)
					c2 = new BlueNoise({weightB: beta * .4 * saliencies[bidx]}).diffuse(pixel, palette[qPixels[bidx]], strength, x, y);
				else
