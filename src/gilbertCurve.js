...
(function(){
	...
	class GilbertCurve {
		...
		constructor(opts, args) {
			...
			if (this.#hasAlpha || (this.#saliencies != null && this.#sortedByYDiff && this.#weight < .03))
				ditherMax = this.#DITHER_MAX / Math.sqrt(this.#weight);
		...
		#ditherPixel(x, y, c2, beta)
		{
			...
			var r1 = (c2 & 0xff),
				...
				b1 = (c2 >>> 16) & 0xff,
				a1 = (c2 >>> 24) & 0xff;
			var gamma = (this.#nMaxColors <= 32 && this.#weight < .01 && this.#weight > .007) ? 1 - beta : beta;
			if (this.#nMaxColors > 4 && this.#Y_Diff(r0, g0, b0, r1, g1, b1) > (gamma * acceptedDiff)) {
				if (this.#margin > 6 || gamma > beta) {
					...
					if (this.#nMaxColors > 32 && this.#saliencies[bidx] < .9)
						...
						else if (this.#nMaxColors >= 32 || this.#Y_Diff(r_pix, g_pix, b_pix, r1, g1, b1) > (gamma * Math.PI * acceptedDiff)) {
				...
				else if (this.#nMaxColors <= 32 && this.#weight >= .004)
			...
			for(var j = 0; j < errLength; ++j) {
				if (Math.abs(error.p[j]) >= this.#ditherMax) {
					...
					if (this.#hasAlpha && this.#saliencies == null) {
						if (Math.abs(error.p[j]) >= (this.#ditherMax * Math.E * Math.PI) || error.p[3] < 0)
