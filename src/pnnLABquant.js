...
(function () {
	...
	class PnnLABQuant {
		#hasSemiTransparency = false; #palette = []; #saliencies;
		...
		#pnnquan(pixels, nMaxColors) {
			...
			this.#saliencies = nMaxColors >= 128 ? null : new Float32Array(pixels.length);
			var saliencyBase = .1;
			...
			for (var i = 0; i < pixels.length; ++i) {
				...
				if (this.#saliencies != null)
					this.#saliencies[i] = saliencyBase + (1 - saliencyBase) * lab1.L / 100 * a / 255;
			}
		...
		quantizeImage() {
			...
			if (this.opts.dithering && this.#saliencies == null && (nMaxColors <= 256 || weight > .99)) {
				this.#saliencies = new Float32Array(pixels.length);
				var saliencyBase = .1;
